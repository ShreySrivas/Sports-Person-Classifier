...
def load_saved_artifacts():
    ...
    with open(os.path.join(ROOT_DIR, 'artifacts', 'class_dictionary.json'), "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v:k for k,v in __class_name_to_number.items()}

    global __model
    if __model is None:
        with open(os.path.join(ROOT_DIR, 'artifacts', 'saved_model.pkl'), 'rb') as f:
...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    face_cascade = cv2.CascadeClassifier(os.path.join(ROOT_DIR, 'haarcascades', 'haarcascade_frontalface_default.xml'))
    eye_cascade = cv2.CascadeClassifier(os.path.join(ROOT_DIR, 'haarcascades', 'haarcascade_eye.xml'))
